...
// static uint16_t cursor_last_x = 0;
// static uint16_t cursor_last_y = 0;
// static uint16_t cursor_last_w = 0;
// static uint16_t cursor_last_h = 1;


// Para texto en modo gráfico
// const font_t *lcd_get_font(void) {
//     return font;
// }
...
static uint8_t font_scale_x = 1;
static uint8_t font_scale_y = 1;

static inline uint8_t clamp_scale(uint8_t v)
{
    if (v < 1) return 1;
    if (v > LCD_MAX_SCALE) return LCD_MAX_SCALE;
    return v;
}

uint8_t lcd_get_font_scale_x(void) { return font_scale_x; }
uint8_t lcd_get_font_scale_y(void) { return font_scale_y; }

void lcd_set_font_scale_x(uint8_t sx) { font_scale_x = clamp_scale(sx); }
void lcd_set_font_scale_y(uint8_t sy) { font_scale_y = clamp_scale(sy); }


uint8_t lcd_get_rows(void)
{
    //return HEIGHT / lcd_cell_h();
    return HEIGHT / ((uint16_t)GLYPH_HEIGHT * lcd_get_font_scale_y());
}

uint8_t lcd_get_max_row(void)
{
...
static inline uint16_t lcd_cell_w(void) { return (uint16_t)font->width * font_scale_x; }
static inline uint16_t lcd_cell_h(void) { return (uint16_t)GLYPH_HEIGHT * font_scale_y; }

#define MAX_GLYPH_W 8
#define MAX_GLYPH_H GLYPH_HEIGHT

static uint16_t char_buffer[MAX_GLYPH_W * LCD_MAX_SCALE * MAX_GLYPH_H * LCD_MAX_SCALE] __attribute__((aligned(4)));
...
void lcd_set_font_scale_x(uint8_t sx);
void lcd_set_font_scale_y(uint8_t sy);
uint8_t lcd_get_font_scale_x(void);
uint8_t lcd_get_font_scale_y(void);

// Opcional: helper
uint8_t lcd_get_rows(void);  // filas disponibles segun escala Y
...
void lcd_scroll_up()
{
    const uint16_t line_h = (uint16_t)GLYPH_HEIGHT * lcd_get_font_scale_y();

    if (lcd_memory_scroll_height == 0) {
        return;
    }

    // Rotate content up by one (scaled) text line
    lcd_y_offset = (lcd_y_offset + line_h) % lcd_memory_scroll_height;
    uint16_t scroll_area_start = lcd_scroll_top + lcd_y_offset;
    ...
    // Clear the new line at the bottom (scaled height)
    lcd_solid_rectangle(background, 0, HEIGHT - line_h, WIDTH, line_h);
}
...
void lcd_scroll_down()
{
    ...
    // Rotate content down by one (scaled) text line
    lcd_y_offset = (lcd_y_offset - line_h + lcd_memory_scroll_height) % lcd_memory_scroll_height;
    uint16_t scroll_area_start = lcd_scroll_top + lcd_y_offset;
    ...
    // Clear the new line at the top (scaled height)
    lcd_solid_rectangle(background, 0, lcd_scroll_top, WIDTH, line_h);
}
...
// Draw a character at the specified position (supports font scaling)
void lcd_putc(uint8_t column, uint8_t row, uint8_t c)
{
    const uint8_t *glyph = &font->glyphs[c * GLYPH_HEIGHT];

    const uint8_t sx = lcd_get_font_scale_x();
    const uint8_t sy = lcd_get_font_scale_y();

    const uint16_t cw = (uint16_t)font->width * sx;      // cell width in pixels
    const uint16_t ch = (uint16_t)GLYPH_HEIGHT * sy;     // cell height in pixels

    uint16_t *out = char_buffer;

    // Render each source glyph row -> expand to sy rows
    for (uint8_t i = 0; i < GLYPH_HEIGHT; i++, glyph++)
    {
        const bool last_row = (i == (GLYPH_HEIGHT - 1));
        const uint8_t bits = *glyph;

        if (font->width == 8)
        {
            // Build 1x row (8 pixels) with existing bold/underscore logic
            uint16_t rowpix[8];

            if (!last_row)
            {
                rowpix[0] = (bits & 0x80) ? foreground : background;
                rowpix[1] = ((bits & 0x40) || (bold && (bits & 0x80))) ? foreground : background;
                rowpix[2] = ((bits & 0x20) || (bold && (bits & 0x40))) ? foreground : background;
                rowpix[3] = ((bits & 0x10) || (bold && (bits & 0x20))) ? foreground : background;
                rowpix[4] = ((bits & 0x08) || (bold && (bits & 0x10))) ? foreground : background;
                rowpix[5] = ((bits & 0x04) || (bold && (bits & 0x08))) ? foreground : background;
                rowpix[6] = ((bits & 0x02) || (bold && (bits & 0x04))) ? foreground : background;
                rowpix[7] = ((bits & 0x01) || (bold && (bits & 0x02))) ? foreground : background;
            }
            else
            {
                // underscore forces the whole last row on (as your original code does)
                rowpix[0] = ((bits & 0x80) || underscore) ? foreground : background;
                rowpix[1] = ((bits & 0x40) || underscore) ? foreground : background;
                rowpix[2] = ((bits & 0x20) || underscore) ? foreground : background;
                rowpix[3] = ((bits & 0x10) || underscore) ? foreground : background;
                rowpix[4] = ((bits & 0x08) || underscore) ? foreground : background;
                rowpix[5] = ((bits & 0x04) || underscore) ? foreground : background;
                rowpix[6] = ((bits & 0x02) || underscore) ? foreground : background;
                rowpix[7] = ((bits & 0x01) || underscore) ? foreground : background;
            }

            // Expand in Y (sy) and X (sx)
            for (uint8_t yy = 0; yy < sy; yy++)
            {
                for (uint8_t x = 0; x < 8; x++)
                {
                    const uint16_t px = rowpix[x];
        ...
        else
        {
            ...
            if (!last_row)
            {
                rowpix[0] = (bits & 0x10) ? foreground : background;
                rowpix[1] = (bits & 0x08) ? foreground : background;
                rowpix[2] = (bits & 0x04) ? foreground : background;
                rowpix[3] = (bits & 0x02) ? foreground : background;
                rowpix[4] = (bits & 0x01) ? foreground : background;
            }
            else
            {
                rowpix[0] = ((bits & 0x10) || underscore) ? foreground : background;
                rowpix[1] = ((bits & 0x08) || underscore) ? foreground : background;
                rowpix[2] = ((bits & 0x04) || underscore) ? foreground : background;
                rowpix[3] = ((bits & 0x02) || underscore) ? foreground : background;
                rowpix[4] = ((bits & 0x01) || underscore) ? foreground : background;
            }
...
{
    // If scaled, fall back to per-character rendering (correct, simpler)
    if (lcd_get_font_scale_x() != 1 || lcd_get_font_scale_y() != 1)
    {
        uint8_t col = column;
        while (*str)
        {
            lcd_putc(col++, row, (uint8_t)(*str++));
        }
        return;
    }

    // --- Original fast path (scale 1:1) ---
    int len = strlen(str);
    int pos = 0;
    while (*str)
    {
...
void lcd_move_cursor(uint8_t column, uint8_t row)
{
    ...
    int cols = (int)lcd_get_columns();
    int rows = (int)lcd_get_rows();

    uint8_t max_col = (cols > 0) ? (uint8_t)(cols - 1) : 0;
    uint8_t max_row = (rows > 0) ? (uint8_t)(rows - 1) : 0;

    if (cursor_column > max_col) cursor_column = max_col;
    if (cursor_row > max_row)   cursor_row = max_row;
}
...
void lcd_draw_cursor()
{
    if (cursor_enabled)
    {
        uint16_t cw = lcd_cell_w();
uint16_t ch = lcd_cell_h();




lcd_solid_rectangle(foreground,
    cursor_column * cw,
    ((cursor_row + 1) * ch) - 1,
    cw, 1);
    }
...
void lcd_erase_cursor()
{
    if (cursor_enabled)
    {
        ...
        lcd_solid_rectangle(background,
            cursor_column * cw,
            ((cursor_row + 1) * ch) - 1,
            cw, 1);
    }
